...
struct _FbTermKeyObjectPrivate {
    int                 npadch;
    short               shift_state;
...
G_DEFINE_TYPE_WITH_PRIVATE (FbTermKeyObject,
                            fbterm_key_object,
                            G_TYPE_INITIALLY_UNOWNED);

static void
fbterm_key_object_init (FbTermKeyObject *key_object)
{
    FbTermKeyObjectPrivate *priv =
            fbterm_key_object_get_instance_private (key_object);

    key_object->priv = priv;
}

static void
fbterm_key_object_class_init (FbTermKeyObjectClass *class)
{
}

static guint16
fbterm_key_object_keypad_keysym_redirect(FbTermKeyObject *key_object,
                                         guint16          keysym)
{
    FbTermKeyObjectPrivate *priv;

    g_return_if_fail (FBTERM_IS_KEY_OBJECT (key_object));

    priv = key_object->priv;
...
FbTermKeyObject *
fbterm_key_object_new ()
{
    return g_object_new (FBTERM_TYPE_KEY_OBJECT,
                         NULL);
}

void
fbterm_key_object_reset (FbTermKeyObject *key_object)
{
    FbTermKeyObjectPrivate *priv;
...
guint16
fbterm_key_object_keycode_to_keysym (FbTermKeyObject *key_object,
                                     guint16          keycode,
                                     char             down)
{
    FbTermKeyObjectPrivate *priv;
    ...
    g_return_val_if_fail (FBTERM_IS_KEY_OBJECT (key_object), K_HOLE);
...
gchar *
fbterm_key_object_keysym_to_term_string (FbTermKeyObject *key_object,
                                         guint16          keysym,
                                         gchar            down)
{
    FbTermKeyObjectPrivate *priv;
    ...
    *buff = '\0';
    ...
    keysym = fbterm_key_object_keypad_keysym_redirect (key_object, keysym);
    int index = 0;
